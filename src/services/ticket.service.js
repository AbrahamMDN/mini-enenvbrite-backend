...
// Función que maneja la lógica de la compra de un ticket
export async function purchase({ eventId, seat }, buyerId) {
  ...
  // Definición de límites en asientos seleccionables
  ...
  // Creación de ticket (con un índice único se garantiza que no haya duplicados)
  ...
  // Generación de QR en PNG, payload y almacenamiento en Supabase 
  // El payload se crea a partir de la información de ID del ticket
  const payload = buildQrPayload(ticket.id);
  // El QR se genera a través del payload
  const png = await generateQrPngBuffer(payload);
  // Creación de ruta de almacenamiento del QR en la DB
  try {
...
// Función que simula el escaneo del ticket existente una única vez
...
// Función que busca un ticket por su ID
